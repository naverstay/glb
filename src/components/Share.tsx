...
const GAME_LINK = 'https://globle.org/worldle';
...
export default function Share({
    ...
    const todaysGuesses = worldleLastWin === today ? worldleUsedGuesses[worldleUsedGuesses.length - 1] : "--";
    ...
    async function copyToClipboard() {
        ...
        let shareString = (!practiceMode && tries > 0 ? `I guessed today’s Worldle in ${tries} ${tries > 1 ? 'tries' : 'try'}:
${worldleEmojiGuesses}🏆
` : `I'm playing Worldle `) + GAME_LINK;

        if (fullStat) {
            shareString =
                `My Worldle Stats:
${worldleGamesWon} - ${localeList[locale]["Stats3"]}
${todaysGuesses} - ${localeList[locale]["Stats2"]}
${todaysGuesses} - ${avgShorthand}
${worldleCurrentStreak} - ${localeList[locale]["Stats4"]}
${worldleMaxStreak} - ${localeList[locale]["Stats5"]}
${GAME_LINK}`;
        }
